...
public class ModRecipes extends RecipeProvider
{
	...
	private void cookFood(Item in, Item out, float exp, int time, Consumer<IFinishedRecipe> consumer) {
		CookingRecipeBuilder.smeltingRecipe(Ingredient.fromItems(in), out, exp, time).addCriterion("has_" + Objects.requireNonNull(in.getRegistryName()).getPath(), hasItem(in)).build(consumer);
	    CookingRecipeBuilder.cookingRecipe(Ingredient.fromItems(in), out, exp, time / 2, IRecipeSerializer.SMOKING).addCriterion("has_" + in.getRegistryName().getPath(), hasItem(in)).build(consumer, new ResourceLocation(BetterEnd.MOD_ID, Objects.requireNonNull(out.getRegistryName()).getPath() + "_from_smoking"));
